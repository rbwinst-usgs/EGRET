...
#' }
estCrossVal<-function(numDays,DecLow,DecHigh, Sample, GetWeights, ApplyWeights, windowY = 7, windowQ = 2, 
...
#   leaveOneOutMatrix <- matrix(rep(NA, numObs))
  ...
  for(i in 1:numObs) {
    ...
    result<-runSurvReg(SampleCrossV$DecYear[i],SampleCrossV$LogQ[i],numDays,DecLow,DecHigh,SampleMinusOne, GetWeights, ApplyWeights,
